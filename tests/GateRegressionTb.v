module Rv32ePcaTb;
  ...
  integer test_op, test_idx, seed;
  reg [31:0] test_a, test_b, test_c;
  function [31:0] reference_result;
    input integer op;
    input [31:0] a,b,c;
    integer lane,ua,ub,sa,sb;
    reg [31:0] result;
    begin
      result=0;
      if(op==5 || op==6) result=c;
      for(lane=0;lane<4;lane=lane+1) begin
        ua=(a>>(lane*8))&255; ub=(b>>(lane*8))&255;
        sa=ua>=128?ua-256:ua; sb=ub>=128?ub-256:ub;
        case(op)
          0: result=result | (((ua+ub)&255)<<(lane*8));
          1: result=result | (((ua-ub)&255)<<(lane*8));
          2: result=result | ((ua>=ub?ua-ub:ub-ua)<<(lane*8));
          4,5: result=result+sa*sb;
          6: result=result+(ua>=ub?ua-ub:ub-ua);
        endcase
      end
      case(op)
        3:result=a^b;
  ...
  initial begin
    ...
    seed=32'h50182026;
    for(test_op=0;test_op<10;test_op=test_op+1)
      for(test_idx=0;test_idx<512;test_idx=test_idx+1) begin
        test_a=$random(seed); test_b=$random(seed); test_c=$random(seed);
        if(test_idx==0) begin test_a=32'h80808080;test_b=32'h80808080; end
        if(test_idx==1) begin test_a=32'h7f7f7f7f;test_b=32'h80808080; end
        run_operation(test_op[3:0],test_a,test_b,test_c,reference_result(test_op,test_a,test_b,test_c));
      end
    $display("WS2 GATE FUNCTIONAL PASS: baseline plus 5120 reference cases; simulation-only MUXI model patch; no SDF");
    $finish;
  end
